...
L.CreditsControl = L.Control.extend({
  ...
  initialize: function (options) {
    ...
    if (!settings.expandcontent)
      throw "L.CreditsControl missing required option: expandcontent";
    if (!settings.imageurl)
      throw "L.CreditsControl missing required option: imageurl";
    if (!settings.tooltip)
      throw "L.CreditsControl missing required option: tooltip";
  ...
  onAdd: function (map) {
    ...
    const contentdivid = `leaflet-credits-control-expandcontent-${Math.round(
      1000000 * Math.random()
    )}`;
  ...
  isExpanded: function () {
    return !L.DomUtil.hasClass(
      this._container,
      "leaflet-credits-control-collapsed"
    );
  },
